...
> Qt5 WebEngine 会缓存入口 HTML 和 ES Module，示例工程已关闭 `QWebEngineProfile`
> HTTP cache，并对入口 URL 追加 `_sdkCacheKey`，集成方照搬时请保留。
...
- 或按主 README 的本地 HTTP Server 方案自行托管 SDK 文件
...
- `QWebEngineProfile` 设置 `NoCache`，并对 `sdk-embed.html` 入口追加版本或时间戳参数
