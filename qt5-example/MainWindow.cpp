...
static const QString SDK_EMBED_URL_DEFAULT =
    ...
    : QMainWindow(parent)
{
    ...
    // 本地验证：若 .env 中设置了 VITE_SDK_EMBED_URL，则覆盖默认 CDN 地址
    const QString localEmbedUrl = get("SDK_EMBED_URL");
    m_sdkEmbedUrl = localEmbedUrl.isEmpty() ? SDK_EMBED_URL_DEFAULT : localEmbedUrl;

    m_stack->setCurrentIndex(0);

    if (!m_clientIdEdit->text().trimmed().isEmpty()
        && !m_secretKeyEdit->text().trimmed().isEmpty()
        && !m_instanceIdEdit->text().trimmed().isEmpty()) {
        QTimer::singleShot(0, this, &MainWindow::onLaunch);
    }
}
